...
function validateWindowName(name) {
    ...
    if (!/^[\w-]+$/u.test(name)) {
        colorLog('❌ Window name can only contain letters, numbers, hyphens, and underscores.', 'red');
        return false;
    }
...
function promptForWindowName() {
    ...
    return new Promise((resolve) => {
        const askForName = () => {
            rl.question("Enter the window name (e.g., 'about', 'settings', 'preferences'): ", (name) => {
        ...
        for (const artifactDir of artifactDirs) {
            ...
            if (fs.existsSync(fullPath)) {
                try {
                    ...
                    colorLog(`   ⚠️  Could not remove ${artifactDir} (this is usually okay)`, 'yellow');
                }
            }
        }
...
function updateFileContent(filePath, windowName) {
    try {
        ...
        content = content.replace(/@app\/window-name-preload/gu, `@app/${windowName}-preload`);
        
        // Replace @app/window-name-renderer with @app/{windowname}-renderer
        content = content.replace(/@app\/window-name-renderer/gu, `@app/${windowName}-renderer`);
...
function showHelp() {
    ...
    colorLog('=====================', 'cyan');
    console.log('');
    colorLog('Creates a new window from the template and replaces placeholders with the desired window name.', 'white');
    console.log('');
    ...
    colorLog('  window-name    Optional. The name for the new window (without spaces or special characters)', 'white');
...
async function main() {
    try {
        ...
        colorLog(`   4. Open the window via IPC: window.electronAPI.invoke('open-window', '${windowName}')`, 'white');
        console.log('');
